...
<aos:onready>
	...
	<script type="text/javascript">
		...
		parent.fnaddtab(row.data.id, '电子文件',
							'archive/data/openSwfFile.jhtml?id='+row.data.pid+'&tid='+row.data.tid+'&type='+row.data.type+'&tablename='+tablename.getValue());
       }
       },
       onUpload : function(){
     ...
     //下载
       ...
       var me =Ext.getCmp("uploadpanel");
		...
		me.store.getAt(file.index).set({"pid":data[i].id_,"tid":data[i].tid});
		 ...
		 upload_url : "${cxt}/archive/upload/archiveZpUpload.jhtml"
		 });
