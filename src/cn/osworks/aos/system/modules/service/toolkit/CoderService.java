	...
	public List<TreeNode> listProjectView(Dto queryDto) {
		...
		if (StringUtils.equalsIgnoreCase("aosroot_", path)) {
			...
			if (StringUtils.equals(coder_project_rootpath_, "0")) {
				...
				path="D:/Workspaces/MyEclipse 10"+queryDto.getString("contextPath");
				//path=ServletContext.getRequest().getServletContext().getContextPath();
			} else {
		...
		if (file.exists()) {
			...
			for (File subFile : subFiles) {
				...
				if (subFile.isDirectory()) {
					// 目录
					treeNode.setA("1");
					if (StringUtils.contains(subPath, "aos\\")) {
						// java package
						treeNode.setIcon("icon34.gif");
						// 目录类型：java源文件包
						treeNode.setB("1");
						String pkgpath = StringUtils.substringAfter(subPath, "aos\\");
					...
					treeNode.setA("2");
					treeNode.setLeaf(true);
					if (StringUtils.endsWith(filename, "aos")) {
